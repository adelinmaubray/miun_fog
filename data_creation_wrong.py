...
def fermentation_creation_wrong():
    ...
    for index, row in df.iterrows():
        ...
        elif not T_1 - T_1_tol/2 <= row['Temperature'] <= T_1 + T_1_tol/2 and not p - p_tol/2 <= row['Pression'] <= p + p_tol/2:
            tmp = 0
        elif not T_1 - T_1_tol/2 <= row['Temperature'] <= T_1 + T_1_tol/2 and not pH - pH_tol/2 <= row['pH'] <= pH + pH_tol/2:
            tmp = 0
